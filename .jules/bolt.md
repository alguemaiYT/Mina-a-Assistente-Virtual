...
## 2024-07-28 - Offloading Synchronous CPU-Bound Audio Decoding to Background Threads
**Learning:** Calling the synchronous, CPU-intensive C-extension function `miniaudio.decode` inside the `async def play` loop blocks the main `asyncio` event loop. This starves other asynchronous tasks (like TTS chunk reception or UI updates) of execution time.
**Action:** Always offload synchronous CPU-bound operations in async contexts to a background worker thread using `asyncio.to_thread`.
