...
## 2024-07-17 - Avoid Blocking Asyncio Event Loop with Synchronous Decoding
**Learning:** `miniaudio.decode` is a CPU-bound operation and blocking the asyncio event loop with it severely hurts the responsiveness of asynchronous programs like `TTSClient`. Offloading it to a background thread prevents playback tasks from halting the entire application.
**Action:** Always wrap synchronous CPU-bound operations in `await asyncio.to_thread` when executing inside async coroutines.
