...
class TestWindowedLayoutAndCLI(unittest.TestCase):
    ...
    def test_main_gui_cli_args_parsing(self):
        """Test R2: main_gui._parse_cli_args with -w, -g, --windowed, --gui, -f, -r."""
        # Test -w (windowed)
        ...
        # Test -g (gui / windowed)
        with patch.object(sys, "argv", ["main_gui.py", "-g"]):
            fullscreen, studio, gravity, windowed = main_gui._parse_cli_args()
            self.assertFalse(fullscreen)
            self.assertTrue(windowed)
        ...
        # Test -f (fullscreen without windowed)
        ...
        # Test -r (gravity)
        with patch.object(sys, "argv", ["main_gui.py", "-r", "right"]):
            fullscreen, studio, gravity, windowed = main_gui._parse_cli_args()
            self.assertEqual(gravity, "right")

    def test_main_cli_gui_launch_detection(self):
        """Test R2: main_cli._check_gui_launch_args detects windowed/gui flags."""
        with patch.object(sys, "argv", ["main_cli.py", "-g"]):
        ...
        with patch.object(sys, "argv", ["main_cli.py", "-r", "right"]):
            self.assertTrue(main_cli._check_gui_launch_args())
